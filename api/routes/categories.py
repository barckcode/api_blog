...
# Routes
@blog_categories.get("/categories", response_model=list[Category_Model])
def get_all_posts():
    return db_connection.execute(categories_table.select()).fetchall()


@blog_categories.get("/categories/{id}", response_model=Category_Model)
def get_post_by_id(id: int):
    return db_connection.execute(categories_table.select().where(categories_table.c.id_categories == id)).first()


@blog_categories.post("/categories", response_model=Category_Model)
...
@blog_categories.put("/categories/{id}", response_model=Category_Model)
